...
function MapRenderer(props) {
    ...
    useEffect(() => {
        ...
        const getXY = (srcX, srcY, exit) => {
            // Gets the new X, Y coordinates for the next room, following the exit
            switch (exit.dir) {
                case '1':
        ...
        const renderRoom = (x, y, room) => {
            ...
            const roomClick = () => {
                // Clicked on a room
                navigate("/map/" + room.vnum);
                setRoomNum(room.vnum);
            }
        ...
        const DrawExit = (srcX, srcY, exit) => {
            ...
            // Direction rotation in radians
            const rotations = {
                '1': 4.71239,  // North
                '4': 1.5708, // South
                '2': 0,  // East
                '8': 3.14159, // West
                '3': -0.785398, // North-east
                '6': 0.785398, // South-east
                '9': 3.92699, // North-west
                '12': 2.35619, // South-west
            };
        ...
        while (toVisit.length > 0) {
            ...
            if (alternativeExists.length > 0) {
                ...
                for (let i = 0; i < alternativeExists.length; i++) {
                    const exit = alternativeExists[i];
                    ...
                    icon.renderer.elem.addEventListener('mouseover', () => {
                        ...
                        document.getElementById('exitName').innerText = exit.name + " (" + exit.vnum + ")";
                    }, false);
                    icon.renderer.elem.addEventListener('click', () => {
                        // Exit was clicked
                        navigate("/map/" + exit.vnum);
                        setRoomNum(exit.vnum);
                    }, false);
        ...
        function addZUI() {
            ...
            zui.translateSurface(500, 500); // Start somewhere nearish the middle... meh
